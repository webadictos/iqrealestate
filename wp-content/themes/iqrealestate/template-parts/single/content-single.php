...
<article id="post-<?php the_ID(); ?>" <?php post_class('post single-entry'); ?> <?php function_exists('wa_article_attributes') ? wa_article_attributes() : ''; ?>>
    <header class="entry-header single-entry__header">
        <ul id="breadcrumbs" class="breadcrumb-list">
            ...
            $ancestors_invertidos = array_reverse($ancestors);


            $ancestors_invertidos[] = $primary_category['primary_category']->term_id;

            foreach ($ancestors_invertidos as $breadrumb_item) {
            ?>

...
class WA_Social_Module extends WA_Module
{
    ...
    public function show_social_icons($opts = array())
    {
        ...
        $social_networks_icons = array(
            ...
            'flipboard' => '<i class="fa-brands fa-flipboard"></i>',
    ...
    public function show_user_profiles($opts = array())
    {
        $default = array(
            'networks' => array(),
            'exclude' => array(),
        ...
        $social_networks = $params['networks'];
        ...
        if (is_array($social_networks) && count($social_networks) > 0) {
            ...
            foreach ($social_networks as $social_network) {
                ...
                echo '<a class="' . $social_network['social'] . '-social-profile__link wa-social-profiles__link" target="_blank" rel="noopener noreferrer" title="' . sprintf(__('Ver perfil en %s', 'wa-theme'), ucfirst($social_network['social'])) . '" href="' . $social_network['url'] . '">' . apply_filters("wa_social_{$social_network['social']}_icon", $social_networks_icons[$social_network['social']]) . '</a>';
    ...
    public function sharebar($postID, $opts)
    {
        ...
        $default = array(
            'networks' => array('pinterest', 'facebook', 'twitter', 'linkedin', 'whatsapp', 'email'),
            'css' => '',
            'link' => '',
            'title' => '',
        );
        ...
        $social_networks = array(
            ...
            'linkedin' => array(
                ...
                'icon' => '<i class="fa-brands fa-linkedin-in"></i></a>',
        ...
        foreach ($networks as $network) {
            ...
            $urlToShare = trim($params['link'] !== "") ? $params['link'] : get_the_permalink($postID);
            $titleToShare = trim($params['title'] !== "") ? $params['title'] : get_the_title($postID);

            //  $urlToShare = $urlToShare . "&utm_source=" . $network . "&utm_medium=social&utm_campaign=sharebuttons";

            if ($current_network !== "") {
                echo "<li>";

                $link = str_replace("{URL}", urlencode($urlToShare), $current_network['link']);
                $link = str_replace("{TITLE}", apply_filters("wa_share_{$network}_title", $titleToShare), $link);


                echo '<a href="' . $link . '" target="_blank" class="' . $network . '-social-share__link wa-social-share__link" title="' . $current_network['share_title'] . '" rel="nofollow noreferrer noopener">' . apply_filters("wa_share_{$network}_icon", $current_network['icon']) . '</a>';
                echo "</li>";
            }
        }
...
/**
 * Funcion para imprimir los links de los perfiles de redes sociales de cada usuario
 */

function wa_show_user_profiles($opts = array())
{
    $wa_theme = $GLOBALS['WA_Theme'];
    ...
    if (is_a($wa_theme, 'WA_Theme')) {
        ...
        if ($social_module = $modulos->module("social")) {
            ...
            $social_module->show_user_profiles($opts);
        }
    }
}

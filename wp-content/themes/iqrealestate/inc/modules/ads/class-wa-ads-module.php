...
class WA_Ads_Module extends WA_Module
{
    ...
    public function init()
    {
        ...
        add_shortcode('wa_ad_slot', array($this, 'shortcode_handler'), 10, 2);
    }
            ...
            $slots_group => array(
                ...
                'wa_group_fields' => apply_filters('wa_theme_get_' . $slots_group . '_fields', array(
                    ...
                    'code' => array(
                        ...
                        'attributes' => array(
                            //'required' => 'required', 
                            'placeholder' => "/" . $this->module_config['network'] . "/" . $this->module_config['prefix'] . "/{identificador}"
                        ),
                    ),
                    'refresh' => array(
    ...
    public function get_size_mappings_options($field)
    {
        ...
        $sizes = array(
            ...
            'superbanner' => 'Super Banner',
            'billboard' => 'Billboard',
            'billboardboxmobile' => 'Billboard Box Mobile',
            'inread' => 'In Read (1x1,300x250)',
            'inread_multiple' => 'In Read secundarios (1x1,300x250)',
            'interstitial' => 'Interstitial',
    ...
    public function get_front_settings($settings)
    {
        ...
        if ($settings['enabled']) {
            ...
            if (!empty($settings['inread_slot'])) {
                $inread_slot = array();
                $inread_slot['id'] = $settings['inread_slot'];
            ...
            if (!empty($settings['multiple_inread_slot'])) {
                $multiple_inread_slot = array();
                $multiple_inread_slot['id'] = $settings['multiple_inread_slot'];
    ...
    public function shortcode_handler($atts, $content = null)
    {
        ...
        if ($params['slot'] !== "") {
            $_slot = wa_theme()->module('ads')->ad_slot($params['slot']);

            ob_start();

            wa_create_ad_slot($_slot, $params);

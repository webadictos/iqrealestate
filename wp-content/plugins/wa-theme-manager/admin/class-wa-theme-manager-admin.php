		...
		if (is_array($metaboxes)) {
			...
			foreach ($metaboxes as $metabox) {
				...
				if (isset($metabox['wa_metabox_fields']) && is_array($metabox['wa_metabox_fields']) && count($metabox['wa_metabox_fields']) > 0) {
					...
					if (isset($metabox['has_tabs']) && $metabox['has_tabs']) {

						$_tabs = array();
			
						if (is_array($metabox['wa_metabox_fields'])) {
							$tabGroups = array(); // Un array para agrupar los campos por 'tab_id'
					
							foreach ($metabox['wa_metabox_fields'] as $field) {
					
								// Verifica si se proporciona un 'tab_id' en el campo, de lo contrario, crea uno basado en uniqid()
								$tab_id = isset($field['tab_id']) ? $field['tab_id'] : 'tab-' . uniqid();
					
								// Crea un grupo para agrupar los campos con el mismo 'tab_id'
								if (!isset($tabGroups[$tab_id])) {
									$tabGroups[$tab_id] = array(
										'id'    => $tab_id,
										'icon' => $field['tab_icon'] ?? '',
								...
								// Agrega el campo al grupo correspondiente
								$tabGroups[$tab_id]['fields'][] = $field['id'];
							}
					
							// Convierte los grupos en un array numerado y lo asigna a $optionPage['tabs']
							$metabox['tabs'] = array_values($tabGroups);
						}
					}
			...
			foreach ($optionsPage as $optionPage) {
				...
				if (isset($optionPage['has_tabs']) && $optionPage['has_tabs']) {
					...
					if (is_array($optionPage['wa_fields'])) {
						...
						// Convierte los grupos en un array numerado y lo asigna a $optionPage['tabs']
						$optionPage['tabs'] = array_values($tabGroups);
					}
				}

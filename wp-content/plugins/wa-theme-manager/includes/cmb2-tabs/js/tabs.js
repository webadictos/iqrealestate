...
(function ($) {
  function toggleFields(tab, form) {
    form.find(tab.data('fields')).each(function () {
      var field = $(this);
      var hasConditionalElements =
        field.find('[data-conditional-id]').length > 0;

      if (hasConditionalElements) {
        field.addClass('cmb-tab-active-item');
      } else {
        field.fadeIn('fast').addClass('cmb-tab-active-item');
      }

      // Support for groups and repeatable fields
      field
        .find('.cmb-repeat-table .cmb-row, .cmb-repeatable-group .cmb-row')
        .addClass('cmb-tab-active-item');
    });
  }
  ...
  if ($('.cmb-tabs').length) {
    $('.cmb-tabs').each(function () {
      // Activate first tab
      if (!$(this).find('.cmb-tab.active').length) {
        var firstTab = $(this).find('.cmb-tab').first();
        firstTab.addClass('active');
        toggleFields(
          firstTab,
          firstTab.closest('.cmb-tabs').next('.cmb2-wrap')
        );
      }
    });
  }
  ...
  $('body').on('click.cmbTabs', '.cmb-tabs .cmb-tab', function (e) {
    ...
    if (!tab.hasClass('active')) {
      ...
      // Hide current active tab fields
      form
        .find(tabs.find('.cmb-tab.active').data('fields'))
        .fadeOut('fast', function () {
          $(this).removeClass('cmb-tab-active-item');
          toggleFields(tab, form);
        });

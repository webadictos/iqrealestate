	...
	class MAG_CMB2_Field_Post_Search_Ajax
	{
		...
		public function render($field, $value, $object_id, $object_type, $field_type)
		{
			...
			echo $field_type->input(array(
				...
				'data-maxitems'	=> $field->args('maxitems') ? $field->args('maxitems') : '1',
				...
				if (isset($value) && !empty($value)) {
					...
					foreach ($value as $val) {
						...
						if ($field->args('object_type') == 'user') {
							...
							$guid 	= get_the_permalink($val);
							$title	= wp_strip_all_tags(get_the_title($val));
							$featured_img_url = get_the_post_thumbnail_url($val, 'horizontal_large');
							$post_date = get_the_date('F j, Y', $val);
						}
						echo '<li>' . $handle;
						if ($featured_img_url)
							echo '<img src="' . $featured_img_url . '">';
						echo '<input type="hidden" name="' . $field_name . '_results[]" value="' . $val . '"><a href="' . $guid . '" target="_blank" class="edit-link">' . $title . '<span class="fecha">' . $post_date . '</span></a><a class="remover"><span class="dashicons dashicons-no"></span><span class="dashicons dashicons-dismiss"></span></a>';
		...
		public function setup_admin_scripts()
		{
			...
			wp_register_script('mag-post-search-ajax', self::url('js/mag-post-search-ajax.js'), array('jquery', 'jquery-autocomplete', 'jquery-ui-sortable'), "1.2.2.2");
			...
			wp_enqueue_style('mag-post-search-ajax', self::url('css/mag-post-search-ajax.css'), array(), "1.2.7.5");
		}
				...
				if ($_POST['object'] == 'user') {
					...
					if ($results->have_posts()) :
						while ($results->have_posts()) : $results->the_post();
							...
							$featured_img_url = get_the_post_thumbnail_url(get_the_ID(), 'horizontal_large');

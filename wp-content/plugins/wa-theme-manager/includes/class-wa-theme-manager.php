...
class Wa_Theme_Manager
{
	...
	private function load_dependencies()
	{
		...
		require_once plugin_dir_path(dirname(__FILE__)) . 'includes/cmb-field-select2/cmb-field-select2.php';
	...
	static function is_single_or_api($params=''){
		// Verificar si la solicitud se realiza a través de la API
		$is_api_request = defined('REST_REQUEST') && REST_REQUEST;

		if ($is_api_request) {
			if(isset($GLOBALS['pID'])){
				if(is_array($params)){
					return in_array($GLOBALS['pID'],$params);
				}else{
					return true;
				}
			}
		} else {
			// No es una solicitud de API, usar la función is_single estándar
			return is_single($params);
		}
	}
}

...
"9992783884"
  ...
  return (
    <div {...blockProps}>
      {selectedContent && (
        <div className="wa-place-item">
          <h3 className="wa-place-item__item--title">
            {selectedContent.title.rendered}
          </h3>
          <ul className="wa-place-item__info">
            {!selectedContent.cmb2.fp_location_fields
              .fp_location_direccion ? null : (
              <li>
                <span className="wa-place-item__info--label">Dirección:</span>{' '}
                <span className="wa-place-item__info--value">
                  {
                    selectedContent.cmb2.fp_location_fields
                      .fp_location_direccion
                  }
            ...
            {!selectedContent.cmb2.fp_location_fields
              ...
              <li>
                <span className="wa-place-item__info--label">Teléfono:</span>{' '}
                <span className="wa-place-item__info--value">
            ...
            !isURL(
              ...
              <li>
                <span className="wa-place-item__info--label">Sitio Web:</span>{' '}
                <span className="wa-place-item__info--value">
                  <a
                    href={
                      selectedContent.cmb2.fp_location_fields.fp_location_web
            ...
            {selectedContent.cmb2.fp_location_fields
              .fp_location_social_networks ? (
              <li className="wa-place-item__social-networks">
                <span className="wa-place-item__info--label">
                  Redes sociales:
                </span>{' '}
                {selectedContent.cmb2.fp_location_fields.fp_location_social_networks.map(
                  (element, index) => (
                    <>
                      <a
                        className={`wa-place-item__info--value wa-place-item__social--${element.social}`}
                        href={element.url}
                        target="_blank"
                        rel="noopener"
                        key={index}
